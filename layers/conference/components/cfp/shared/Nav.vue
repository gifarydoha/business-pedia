  ...
  <header class="sticky top-0 z-50 flex w-full flex-col shadow-sm">
    <div class="bg-white">
      <div class="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:gap-4 sm:px-6 sm:py-4">
        <NuxtLink to="/" class="group flex min-w-0 flex-1 items-center gap-2 sm:gap-4">
          <NuxtImg v-if="logo && !logoError" :src="logo" :alt="orgName"
            class="h-10 w-auto shrink-0 object-contain sm:h-12 md:h-14" @error="logoError = true" />
          <NuxtImg v-else src="/images/cfp-logo.png" :alt="orgName"
          ...
          <div class="flex min-w-0 flex-col justify-center">
            ...
            <div
              ...
              <Icon name="ph:map-pin-fill" class="size-3 shrink-0 sm:size-3.5 md:size-4" />
        ...
        <button class="shrink-0 text-cfp-olive md:hidden" aria-label="Toggle menu" @click="open = !open">
    ...
    <div class="hidden bg-cfp-yellow md:block">
      <div class="mx-auto flex max-w-7xl items-center justify-between pl-6">
        <div class="flex items-center gap-6 overflow-x-auto">
          <NuxtLink v-for="l in navLinks" :key="l.label" :to="l.href"
        ...
        <div v-if="isLoggedIn" id="user-dropdown-wrapper" class="relative self-stretch">
          <button
            ...
            <span
              ...
              <img v-if="avatarUrl" :src="avatarUrl" :alt="authUser?.name" class="size-full object-cover">
              <span v-else class="text-xs font-bold text-cfp-olive">{{ userInitials }}</span>
            </span>
            <span class="max-w-30 truncate">{{ authUser?.name }}</span>
            <Icon name="ph:caret-down-bold" class="size-3.5 shrink-0 transition-transform duration-200"
          ...
          <Transition enter-active-class="transition duration-150 ease-out" enter-from-class="opacity-0 -translate-y-1"
            ...
            <div v-show="userDropdownOpen"
              ...
              <nav class="py-1">
                <NuxtLink v-for="item in USER_MENU_ITEMS" :key="item.href" :to="item.href"
                  ...
                  <Icon :name="item.icon" class="size-4 shrink-0 text-cfp-olive/70" />
                  {{ item.label }}
                </NuxtLink>
              </nav>
              <div class="border-t border-gray-100 py-1">
                <button
                  ...
                  @click="handleLogout">
                  <Icon name="ph:sign-out-bold" class="size-4 shrink-0" />
                  Logout
                </button>
              </div>
            </div>
    ...
    <div v-show="open" class="flex flex-col gap-3 bg-cfp-yellow px-6 pt-2 pb-4 md:hidden">
      <NuxtLink v-for="l in navLinks" :key="l.label" :to="l.href"
        class="py-2 font-poppins text-sm tracking-wide uppercase transition-colors hover:text-cfp-olive"
        :class="isActive(l.href) ? 'text-cfp-olive font-semibold' : 'text-white'" @click="open = false">
        {{ l.label }}
      </NuxtLink>
      ...
      <template v-if="isLoggedIn">
        <button class="flex w-full items-center justify-between py-2 font-poppins text-sm font-semibold text-white"
        ...
        <Transition enter-active-class="transition-all duration-200 ease-out" enter-from-class="opacity-0 max-h-0"
          enter-to-class="opacity-100 max-h-60" leave-active-class="transition-all duration-150 ease-in"
          leave-from-class="opacity-100 max-h-60" leave-to-class="opacity-0 max-h-0">
          <div v-show="mobileAccountOpen" class="overflow-hidden rounded-xl bg-white/10 backdrop-blur-sm">
            <NuxtLink v-for="item in USER_MENU_ITEMS" :key="item.href" :to="item.href"
              ...
              <Icon :name="item.icon" class="size-4 shrink-0 text-white/70" />
              {{ item.label }}
            </NuxtLink>
