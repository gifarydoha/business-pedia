  ...
  <nav class="sticky top-0 z-50 border-b border-cfp-olive/10 bg-white shadow-sm">
    <div class="mx-auto flex max-w-6xl items-center justify-between gap-6 px-6 py-3">
      <NuxtLink to="/" class="group flex items-center gap-3">
        <CfpLogo class="h-10 w-auto" />
        <span
          class="font-lora text-lg font-bold tracking-tight text-cfp-olive transition-colors group-hover:text-cfp-red">SBAC
          2026</span>
      </NuxtLink>
      ...
      <!-- Mobile hamburger -->
      <button class="text-cfp-olive md:hidden" aria-label="Toggle menu" @click="open = !open">
    ...
    <!-- Mobile menu -->
    <div v-show="open" class="flex flex-col gap-3 bg-cfp-olive-dark px-6 pb-4 md:hidden">
      <NuxtLink v-for="l in navLinks" :key="l.label" :to="l.href"
        class="py-1 font-poppins text-sm transition-colors hover:text-white"
        :class="isActive(l.href) ? 'text-cfp-yellow font-medium' : 'text-white/80'" @click="open = false">
        {{ l.label }}
      </NuxtLink>
      <NuxtLink to="/submit-paper"
